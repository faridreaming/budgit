...
export default function EditTransaksi({
  ...
  onUpdateRequest: (transaksi: Transaksi) => void
  onCancel: () => void
  transaksi: Transaksi
}) {
  const step = 500
  const min = 0
  const prevJenisRef = useRef<string>('pengeluaran')
  ...
  useEffect(() => {
    if (transaksi) {
      const selectedKategori = kategori.find((k) => k.id === transaksi.kategori_id)
      const jenis = selectedKategori?.jenis || 'pengeluaran'
      ...
      form.reset({
        ...
        jenis: jenis,
        kategori: transaksi.kategori_id.toString(),
        deskripsi: transaksi.deskripsi || '',
      })

      // Update the ref with the initial jenis value
      prevJenisRef.current = jenis
    }
  }, [transaksi, form])
  ...
  useEffect(() => {
    ...
    if (jenisValue !== prevJenisRef.current) {
      form.setValue('kategori', '')
      prevJenisRef.current = jenisValue
    }
  }, [jenisValue, form])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="dark:bg-card sm:max-w-xl">
        ...
        <DialogDescription className="sr-only">Edit transaksi Anda.</DialogDescription>
        <Form {...form}>
